...
CREATE
OR REPLACE FUNCTION public.get_user_documents(id_arg uuid) RETURNS TABLE(
    id uuid,
    document_type text,
    document_url text,
    document_name text,
...
SET
    search_path TO 'public' AS $ function $ BEGIN RETURN QUERY
SELECT
    l.id,
    CASE
...
CREATE
OR REPLACE FUNCTION public.get_user_investments(id_arg uuid) RETURNS TABLE(
    id uuid,
...
SET
    search_path TO 'public' AS $ function $ WITH user_contacts AS (
        SELECT
            id
        FROM
            contacts
...
CREATE
OR REPLACE FUNCTION public.get_user_investments_by_id(id_arg uuid, investment_id_arg uuid) RETURNS TABLE(
    id uuid,
    purchase_amount text,
    investment_type text,
    valuation_cap text,
...
CREATE
OR REPLACE FUNCTION public.get_user_links(id_arg uuid) RETURNS SETOF links LANGUAGE plpgsql SECURITY DEFINER AS $ function $ BEGIN RETURN QUERY
SELECT
    *
FROM
    links
...
CREATE
OR REPLACE FUNCTION public.get_user_links_with_views(id_arg uuid) RETURNS TABLE(
    id uuid,
    created_at timestamp with time zone,
    created_by uuid,
    url text,
...
CREATE
OR REPLACE FUNCTION public.select_link(link_id uuid) RETURNS TABLE(
    id uuid,
    created_at timestamp with time zone,
    url text,
    password text,
...
CREATE
OR REPLACE FUNCTION public.update_link(
    link_id uuid,
    user_id uuid,
    url_arg text,
    password_arg text,
    ...
    filename_arg text
) RETURNS void LANGUAGE plpgsql SECURITY DEFINER AS $ function $ BEGIN
UPDATE
    public.links
SET
